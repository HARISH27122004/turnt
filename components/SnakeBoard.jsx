...
export default function SnakeBoard({ snake, food, gameOver, onRestart }) {
        ...
        {Array.from({ length: SNAKE_ROWS }, (_, r) => (
          <div key={r} className={styles.row}>
            {Array.from({ length: SNAKE_COLS }, (_, c) => {
              ...
              if (isFood) return <div key={c} className={styles.food} />;
              if (isHead) return <div key={c} className={styles.head} />;
              if (isSnake) {
                const opacity = Math.max(0.5, 1 - snakeIdx / snake.length * 0.4);
                return <div key={c} className={styles.snake} style={{ opacity }} />;
              }
